...
void real_main() {
  char buf[0x100] = "The normal buf";
  struct stack_func_1 s = {{0},0};
  s.where = buf;
  receive_n(0, &s, 0x24);
  send_all(1, s.where, 4);
  *s.where = '\x00';
}

int main() {
  char buf[0x100] = "Hello this is a test program where you control a pointer to transmit\n";
